...
exports.handler = async (event, context) => {
    // Get city and eventApiKeyString from query parameters
    const { eventApiKeyString, city } = event.queryStringParameters;

    if (!eventApiKeyString || !city) {
        return {
            statusCode: 400,
            body: JSON.stringify({ error: "Missing 'eventApiKeyString' or 'city' query parameter." }),
    ...
    params.append('location', city); // Use the city from the query parameter
    params.append('gl', 'au');
    params.append('hl', 'en');
    ...
    try {
        ...
        if (!data.events_results || !Array.isArray(data.events_results)) {
            console.warn("[Netlify Function] No 'events_results' array found in SerpApi response for city:", city);
        ...
        console.log(`[Netlify Function] Processed ${processedEvents.length} events for ${city}`);
        ...
        console.error(`[Netlify Function] Error fetching from SerpApi for city ${city}:`, error);
        return { statusCode: 500, body: JSON.stringify({ error: `Failed to fetch events via proxy for ${city}: ${error.message}` }), headers: { 'Content-Type': 'application/json' } };
    }
};
