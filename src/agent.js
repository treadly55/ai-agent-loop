...
// --- System Prompt Updated: Force Tool Call ---
const systemPrompt = `
You are "Weekend Away," an AI assistant. Your SOLE initial task is to fetch event data for Sydney. Your ultimate goal is to recommend the 2 most exciting-sounding events from that data.

Your process MUST be followed rigidly:

**Step 1: Fetch Event Data (MANDATORY FIRST ACTION)**
* You will be given an 'eventKey' string.
* Your VERY FIRST action, without any other thought or preamble, MUST be to use the 'getEvents' tool.
* You MUST use "Sydney" for the 'city' argument and the exact 'eventKey' string provided to you for the 'eventKey' argument in the tool call.
* Output this action in the format: Action: getEvents: {"city": "Sydney", "eventKey": "the_provided_event_key"}\nPAUSE
...
* Your entire final output message should consist ONLY of the following structure:
    * An introductory sentence like "Based on the events for Sydney, here are the two that sound most exciting!".
...
Interaction Flow:
1.  Thought: (Optional for first step, as the action is fixed) My only first step is to call getEvents.
2.  Action: getEvents: {"city": "Sydney", "eventKey": "provided_event_key"}\nPAUSE
3.  PAUSE
4.  Observation: (List of events from the tool)
5.  Thought: Now I will analyze these events and pick the top 2 most exciting.
6.  Final Output: (Formatted HTML recommendations, no "Thought" or "Action")

Example of the ONLY valid format for your final response message:
Based on the events for Sydney, here are the two that sound most exciting!
...
export async function runWeekendAgent(eventApiKeyString, progressCallback) {
    ...
    // The user query now just reinforces the task, the prompt handles the "how".
    const userQuery = `Fetch events for Sydney using event key '${eventApiKeyString}' and then recommend the top 2 most exciting ones. Follow the system prompt's formatting instructions precisely for the final output.`;
    ...
    const MAX_ITERATIONS = 3; // 1. Call getEvents, 2. AI processes and responds.
    ...
    try {
        for (let i = 0; i < MAX_ITERATIONS; i++) {
            ...
            const response = await openai.chat.completions.create({
                model: "gpt-3.5-turbo",
                messages: messages,
                temperature: 0.1, // Lower temperature for more predictable adherence to instructions
            });

            const responseMessage = response.choices[0].message;
            const responseText = responseMessage.content;

            console.log("OpenAI Raw Response:", responseText); // This is key to see what AI is outputting
            if (!responseText) {
            ...
            if (actionMatch) {
                ...
                try {
                    ...
                    // Log that we are about to call the function
                ...
                try {
                    ...
                    // We can add a log right before it.
                    ...
                    console.log(`Observation from ${actionName}:`, toolResult); // Log the observation
                } catch (e) {
                ...
                // No action found, assume this is the final formatted response
                progressCallback("Agent finished. Processing final answer.");
                console.log("Final response from AI (expected format without 'Answer:' prefix):", responseText);
                return responseText.trim(); // This will be output to the results area
            }
        }
